...
We can then do stuff to this `Entity` by chaining a `World.OnAwake()` or `World.OnStart()` call. These two methods act just like `Entity.OnAwake()` `Entity.OnStart()`, but are called after every single `Entity` has received the callback for the corrosponding event method.
```cs
var world = new World(window, keyboard, graphics, camera, renderer)
